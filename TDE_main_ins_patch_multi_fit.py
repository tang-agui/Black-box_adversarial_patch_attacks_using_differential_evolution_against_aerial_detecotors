...
def calculate_fitness(target_image, population, tru_labels):
    ...
    for b in range(population_size):
        ...
        # save_name_instan = 'target_set/instan_patch/patch_test_save/patch_save_numpy/kron/inst_patch.png'
        ...
        #   save这里的attack_image观察
        # save_name_add = 'target_set/instan_patch/patch_test_save/patch_save_numpy/kron/ins_patch_add.png'
        ...
        if len(boxes_attack) == 0:
            ...
            #   2023-08-16：这里f_score是个向量，但是元素都大于0
...
def fitness_selection(fitness):
    ...
    fitness_len = []
    for i, items in enumerate(fitness):
        if (len(items) == 1 and items[0] == -1):
            #   2023-08-16
            '''
            如果某个个体的预测长度为1，且元素为-1，这里的-1在calculate_fitness中，
            满足
            if len(boxes_attack) == 0:
                fitness.append([-1])
            也是只在这里才会提前满足停止条件，仍然是长度
            '''
            fitness_min_value = -1
            ...
            #   这里因为后面还会赋值，那这里没意义啊？？
            #   ！！！！！！！！！！！！！！！！！！
            #   虽然没意义，但是最终殊途同归，经过下面的操作后，仍然会赋值为1！
            print("trigger !", "iteration : ", i, "fitness_min_value : ", fitness_min_value)
            return i, fitness_min_value
    ...
    #   最大值最小的个体，selected_index是个索引，所以fitness_index也是个索引
...
def FDE(clean_image, tru_label):
    ...
    for step in range(generations):
        ...
        if fitness_min_value <= 0:
            #   if min()
            print("break step : ", step, "fitness_min_value : ", fitness_min_value)
            break
        #   变异
        ...
        #   2023-08-16，这里得到的fitness是个向量
        population, fitness = selection(
            clean_image, Cpopulation, population, fitness, tru_label)
        '''
        这里的selection，本质上是对population和Cpopulation进行了选择
        根据对应位置的适应度值fitness，而在这里，fitness函数计算的是每个个体
        的检测长度，因此在selection中直接根据length进行判断
        #############################################################
        然后selection中会同步计算Cpopulation中的适应度值，这里也会进行替换
        即最终population会融合population和Cpopulation，并且fitness也是
        #############################################################
        然后再对此时的population进行判断，如果其中有满足终止条件的个体，则终止
        为下面的fitness_selection()过程
        '''
        #   2023-08-16，这里得到的fitness是个向量，所以需要下面的fitness_selection再选择
        #   这里根据父代和offspring代进行筛选，得到子代，
        fitness_index, fitness_min_value = fitness_selection(fitness)
        #   然后对子代中的个体进行筛选
        #   2023-08-16：其实这里，只要最小长度不小于0就还会迭代，并不需要筛选个体
        Best_indi = population[fitness_index]
